...
function handleDetail(data) {
  ...
  const cmdHandlers = {
    [REQUEST_CMD.CONNECT]: () => {
      const remote = this.remote = net.connect(config.serverPort, pick(config.serverHost))
      ...
      remote.on('connect', () => {
         ...
         * 对返回数据解密
         */
        const decipher = encrypt.getDecipher(config.password, config.iv)
        remote.pipe(decipher).pipe(this)
        // 主要目的是将host跟port加密发送
        remote.write(encrypt.encrypt(reply, config.password, config.iv))

        /**
         * 加密发送数据
         */
        const cipher = encrypt.getCipher(config.password, config.iv)
